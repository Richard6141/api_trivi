...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_422_sent_deleting_non_existing_question(self):
        res = self.client().delete('/questions/moi')
        data = json.loads(res.data)
    ...
    def test_add_question(self):
        ...
        start_total_questions = len(Question.query.all())
        res = self.client().post('/questions', json=new_question)
        data = json.loads(res.data)
        end_total_questions = len(Question.query.all())

        self.assertEqual(res.status_code, 200)
        self.assertEqual(data["success"], True)
        self.assertEqual(end_total_questions, start_total_questions + 1)
    ...
    def test_404_get_questions_per_category(self):
        res = self.client().get('/categories/moi/questions')
        data = json.loads(res.data)
